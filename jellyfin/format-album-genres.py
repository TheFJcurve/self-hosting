...
def update_nfo_file(nfo_path, dry_run=False, backup=True, force_genres=False):
    try:
...
def process_directory(music_dir, dry_run=False, backup=True, force_genres=False):
    ...
    for nfo_file in nfo_files:
        print(f"\nProcessing: {nfo_file}")
        success, changed, message = update_nfo_file(nfo_file, dry_run, backup, force_genres)
...
def main():
    parser = argparse.ArgumentParser(
        description='Update Jellyfin music .nfo files with MusicBrainz genres'
    )
    parser.add_argument(
        'music_dir',
    ...
    parser.add_argument(
        '--force',
        action='store_true',
        help='Force refetch all genres from MusicBrainz, replacing existing ones'
    )
